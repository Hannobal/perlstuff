...
if($#ARGV<7) {
  ...
  print "6. length of vector a in Angstroms (x-direction)\n";
  print "7. length of vector b in Angstroms\n";
  print "8. angle between vectors a an b in degree\n";
  print "[9. offset angle to rotate lattice in degree]\n";
  print "[10. mol shift vector x component in Angstroms]\n";
  print "[11. mol shift vector y component in Angstroms]\n";
  print "[12. (zero/mono/bi/tri)-dentat binding]\n";
  print "[13. additional distance to surface]\n";
  exit 1;
}

$lena  = $ARGV[5]; # first index: new vector, second index: old vector
$lenb  = $ARGV[6];
$angle = $ARGV[7];
if($lena<2 or $lenb<2) {
  print "error: space between rows must be at least 2 A\n";
  exit 1;
}

if($#ARGV>7){ $offsetangle   =$ARGV[8];  } else { $offsetangle   =0; }
if($#ARGV>8){ $molshiftvec[0]=$ARGV[9];  } else { $molshiftvec[0]=0; }
if($#ARGV>9){ $molshiftvec[1]=$ARGV[10]; } else { $molshiftvec[1]=0; }
if(lc(substr($ARGV[11],0,4)) eq "zero") {
  $bindingmode = 0;
} elsif(lc(substr($ARGV[11],0,4)) eq "mono") {
  $bindingmode = 1;
} elsif(lc(substr($ARGV[11],0,2)) eq "bi") {
  $bindingmode = 2;
} elsif(lc(substr($ARGV[11],0,3)) eq "tri") {
  $bindingmode = 3;
} else {
  $bindingmode = undef;
}
if($#ARGV>11){ $surfdist=$ARGV[12]; } else { $surfdist=0; }

$molperiodicvec[0][0] = $lena*cos($offsetangle/180.0*pi);
$molperiodicvec[0][1] = $lena*sin($offsetangle/180.0*pi);
$molperiodicvec[1][0] = $lenb*cos(($angle+$offsetangle)/180.0*pi);
$molperiodicvec[1][1] = $lenb*sin(($angle+$offsetangle)/180.0*pi);
...
for($i=0;$i<=$#molpos;$i++) {
  ...
  if($bindingmode>0) {
    $molpos[$i][0]=$xpos/$bindingmode+$surfdist;
    $molpos[$i][1]=$ypos/$bindingmode+$surfdist;
...
#       for($x=-1;$x<2;$x++) {
# 	for($y=-1;$y<2;$y++) {
# 	  $dx=$molpos[$i][0]-$candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0];
# 	  $dy=$molpos[$i][1]-$candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	  $dist=sqrt($dx**2+$dy**2);
# 	  if($dist<$dmin) {
# 	    $remove[0][0] = $j;
# 	    $remove[0][1] = $candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0];
# 	    $remove[0][2] = $candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	    $remove[0][3] = $candidate[$j][3];
# 	    $dmin=$dist;
# 	  }
# 	}
#       }
...
#     $molpos[$i][2]=$remove[0][3];
#     $mol_deltanuments[$foundmol2name]++;
#     $mol_deltanuments[$candidate[$remove[0][0]][4]]--;
#     $ohremoved++;
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[0][0],1);
#   }
...
#       for($x=-1;$x<2;$x++) {
# 	for($y=-1;$y<2;$y++) {
# 	  $dx=$molpos[$i][0]-$candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0]+0.5*$ohperiodicvec[0][0];
# 	  $dy=$molpos[$i][1]-$candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	  $dist=sqrt($dx**2+$dy**2);
# 	  if($dist<$dmin) {
# 	    $remove[0][0] = $j;
...
# 	  if($dist<$dmin) {
# 	    $remove[1][0] = $j;
# 	    $remove[1][1] = $candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0];
# 	    $remove[1][2] = $candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	    $remove[1][3] = $candidate[$j][3];
# 	    $dmin=$dist;
# 	  }
# 	}
#       }
...
#     $molpos[$i][2]=($remove[0][3]+$remove[1][3])/2;
#     @remove = sort { $a->[0] <=> $b->[0] } @remove;
#     $mol_deltanuments[$foundmol2name]++;
#     $mol_deltanuments[$candidate[$remove[0][0]][4]]-=2;
#     $ohremoved+=2;
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[1][0],1);
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[0][0],1);
#   }
...
#       for($x=-1;$x<2;$x++) {
# 	for($y=-1;$y<2;$y++) {
# 	  $dx=$molpos[$i][0]-$candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0]+$ohperiodicvec[0][0];
# 	  $dy=$molpos[$i][1]-$candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	  $dist=sqrt($dx**2+$dy**2);
# 	  if($dist<$dmin) {
# 	    $remove[1][0] = $j;
...
#       for($x=-1;$x<2;$x++) {
# 	for($y=-1;$y<2;$y++) {
# 	  $dx=$molpos[$i][0]-$candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0]+$ohperiodicvec[1][0];
# 	  $dy=$molpos[$i][1]-$candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1]+$ohperiodicvec[1][1];
# 	  $dist=sqrt($dx**2+$dy**2);
# 	  if($dist<$dmin) {
# 	    $remove[2][0] = $j;
# 	    $remove[2][1] = $candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0];
# 	    $remove[2][2] = $candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1];
# 	    $remove[2][3] = $candidate[$j][3];
# 	    $dmin=$dist;
# 	  }
# 	}
#       }
...
# #     $molpos[$i][0]=($remove[0][1]+$remove[1][1])/2;
# #     $molpos[$i][1]=($remove[0][2]+$remove[1][2])/2;
#     $molpos[$i][2]=($remove[0][3]+$remove[1][3]+$remove[2][3])/3;
#     @remove = sort { $a->[0] <=> $b->[0] } @remove;
#     $mol_deltanuments[$foundmol2name]++;
#     $mol_deltanuments[$candidate[$remove[0][0]][4]]-=3;
#     $ohremoved+=3;
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[2][0],1);
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[1][0],1);
#     @removedmols[$#removedmols+1] = splice(@candidate,$remove[0][0],1);
...
# #       for($x=-1;$x<2;$x++) {
# # 	for($y=-1;$y<2;$y++) {
# # 	  $dx=$molpos[$i][0]-$candidate[$j][1]+$x*$vec[0][0]+$y*$vec[1][0]+$ohperiodicvec[1][0]+$ohperiodicvec[0][0];
# # 	  $dy=$molpos[$i][1]-$candidate[$j][2]+$x*$vec[0][1]+$y*$vec[1][1]+$ohperiodicvec[1][1]+$ohperiodicvec[0][1];
# # 	  $dist=sqrt($dx**2+$dy**2);
# # 	  if($dist<$dmin) {
# # 	    $oxidize=$j;
# # 	    $dmin=$dist;
...
# #       }
# #     }
# #     $mol_deltanuments[$candidate[$oxidize][4]]--;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][0]="OA";
# #     $cdata[$foundox][$mol_numents[$foundox]][0][1]=$candidate[$oxidize][1];
# #     $cdata[$foundox][$mol_numents[$foundox]][0][2]=$candidate[$oxidize][2];
# #     $cdata[$foundox][$mol_numents[$foundox]][0][3]=$candidate[$oxidize][3];
# #     $cdata[$foundox][$mol_numents[$foundox]][0][4]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][5]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][6]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][7]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][8]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][9]=0;
# #     $cdata[$foundox][$mol_numents[$foundox]][0][10]=$molindex;
# #     @removedmols[$#removedmols+1] = splice(@candidate,$oxidize,1);
# #     $molindex++;
# #     $mol_numents[$foundox]++;
#   }
# } else {
