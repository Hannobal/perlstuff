...
die "**** error: no numeric directories and no HISTORY file in work directory were found" if($#directories<0);
...
foreach $dir (@directories) {
  ...
  open(CONTROL, "<", "$dir/CONTROL") or print "**** warning: file CONTROL was not found in $dir\n";
  while(<CONTROL>) {
    if(/\s*timestep/) {
  ...
  open($fhhist, "<", "$dir/HISTORY") or print "**** warning: file HISTORY was not found in $dir\n";
  $starttime=$time if($heatup);
  whilereadhist : while(1) {
    last if(read_history_timestep($fhhist,0,0) != 0);
...
#     }
    ...
    for($i=1;$i<$stepframe;$i++) {
      last whilereadhist if (not skip_history_timestep($fhhist)==0);
    }
  } # end while
  close($fhhist);
} # end for directories
